...
namespace POSA.Forms
{
    public partial class UpdateSize : Form
    {
        ...
        private async void UpdateCategory_Load(object sender, EventArgs e)
        {
            ...
            sb.Where("ID=@ID");
            var param = new { ID = ID };
            var builderTemp = sb.AddTemplate("SELECT /**select**/ FROM SIZES /**where**/");
        ...
        public async Task<bool> Update(string id)
        {
            ...
            sb.Where("ID=@ID");
            var param = new { ID = ID };
            var builderTemp = sb.AddTemplate($"UPDATE SIZES  SET NAME='{tbSize.Text ?? ""}',DESCRIPTION='{tbDescription.Text ?? ""}',CHANGEDBY='{settings.LastSuccesfullyLoggedUser}',CHANGEDATE=GETDATE() /**where**/");
            await using var conn = new SqlConnection(settings.Sql.ConnectionString());
            conn.Open();
