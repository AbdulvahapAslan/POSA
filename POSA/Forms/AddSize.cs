...
namespace POSA.Forms
{
    public partial class AddSize : Form
    {
        ...
        public async void RefreshDataGrid()
        {
            ...
            var builderTemp = sb.AddTemplate("SELECT /**select**/ FROM SIZES WHERE ID>1");
        ...
        public async void DeleteFromDB(string id)
        {
            ...
            var settings = Setting.Get();
            var sb = new SqlBuilder();
            var builderTemp = sb.AddTemplate($"DELETE FROM SIZES WHERE ID = {id}");
            await using var conn = new SqlConnection(settings.Sql.ConnectionString());
            conn.Open();
        ...
        private async void btnSave_Click(object sender, EventArgs e)
        {
            var settings = Setting.Get();
            var sb = new SqlBuilder();
            var builderTemp = sb.AddTemplate("INSERT INTO SIZES(NAME,DESCRIPTION,CREATEDBY,CREATEDATE) VALUES(@NAME,@DESCRIPTION,@CREATEDBY,GETDATE())");
            var param = new
            {
                NAME = tbSize.Text,
        ...
        private void dgvMain_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                var us = new UpdateSize(dgvMain.Rows[e.RowIndex].Cells["ID"].Value.ToString());
