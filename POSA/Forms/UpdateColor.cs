...
namespace POSA.Forms
{
    public partial class UpdateColor : Form
    {
        ...
        private async void UpdateCategory_Load(object sender, EventArgs e)
        {
            ...
            sb.Where("ID=@ID");
            var param = new { ID = ID };
            var builderTemp = sb.AddTemplate("SELECT /**select**/ FROM COLORS /**where**/");
        ...
        public async Task<bool> Update(string id)
        {
            ...
            sb.Where("ID=@ID");
            var param = new { ID = ID };
            var builderTemp = sb.AddTemplate($"UPDATE COLORS  SET NAME='{tbColor.Text ?? ""}',DESCRIPTION='{tbDescription.Text ?? ""}',CHANGEDBY='{settings.LastSuccesfullyLoggedUser}',CHANGEDATE=GETDATE() /**where**/");
            await using var conn = new SqlConnection(settings.Sql.ConnectionString());
            conn.Open();
        ...
        private void UpdateCategory_MouseDown(object sender, MouseEventArgs e)
        {
