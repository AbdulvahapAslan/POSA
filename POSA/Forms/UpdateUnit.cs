...
namespace POSA.Forms
{
    public partial class UpdateUnit : Form
    {
        ...
        private async void UpdateCategory_Load(object sender, EventArgs e)
        {
            ...
            sb.Where("ID=@ID");
            var param = new { ID = ID };
            var builderTemp = sb.AddTemplate("SELECT /**select**/ FROM UNITS /**where**/");
        ...
        public async Task<bool> Update(string id)
        {
            ...
            sb.Where("ID=@ID");
            var param = new { ID = ID };
            var builderTemp = sb.AddTemplate($"UPDATE UNITS  SET NAME='{tbUnit.Text ?? ""}',DESCRIPTION='{tbDescription.Text ?? ""}',CHANGEDBY='{settings.LastSuccesfullyLoggedUser}',CHANGEDATE=GETDATE() /**where**/");
            await using var conn = new SqlConnection(settings.Sql.ConnectionString());
            conn.Open();
