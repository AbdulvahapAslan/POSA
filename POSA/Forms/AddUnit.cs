...
namespace POSA.Forms
{
    public partial class AddUnit : Form
    {
        ...
        private async void AddUnit_Load(object sender, EventArgs e)
        {
        ...
        public async void RefreshDataGrid()
        {
            ...
            var builderTemp = sb.AddTemplate("SELECT /**select**/ FROM UNITS WHERE ID>1");
        ...
        public async void DeleteFromDB(string id)
        {
            ...
            var settings = Setting.Get();
            var sb = new SqlBuilder();
            var builderTemp = sb.AddTemplate($"DELETE FROM UNITS WHERE ID = {id}");
            await using var conn = new SqlConnection(settings.Sql.ConnectionString());
            conn.Open();
        ...
        private async void btnSave_Click(object sender, EventArgs e)
        {
            var settings = Setting.Get();
            var sb = new SqlBuilder();
            var builderTemp = sb.AddTemplate("INSERT INTO UNITS(NAME,DESCRIPTION,CREATEDBY,CREATEDATE) VALUES(@NAME,@DESCRIPTION,@CREATEDBY,GETDATE())");
            var param = new
            {
                NAME = tbUnit.Text,
        ...
        private void dgvMain_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                var uu = new UpdateUnit(dgvMain.Rows[e.RowIndex].Cells["ID"].Value.ToString());
                DialogResult dr = uu.ShowDialog();
        ...
        private void AddUnit_MouseDown(object sender, MouseEventArgs e)
        {
